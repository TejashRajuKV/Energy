...
import { TrendingDown, Wind, Monitor, Lightbulb, Wifi, Coffee, Download, Share2, Sliders, ChevronDown, ChevronUp, Info } from 'lucide-react';
...
export default function AnalysisResultPage() {
  ...
  const handleExportPdf = useReactToPrint({
    ...
    contentRef: printRef,
    documentTitle: `ghostload-report-${analysisId}`,
  });
  ...
  const handleExportCsv = async () => {
    try {
      const res = await api.post(`/api/reports/${analysisId}/export/csv`, {}, { responseType: 'blob' });
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a');
      a.href = url; a.download = `ghostload-${analysisId}.csv`; a.click();
  ...
  if (!analysis) return (
    <AppLayout>
      <div className="empty-state"><h2>Analysis not found</h2><Link to="/app" className="btn btn-primary" style={{ marginTop: '1rem' }}>Go to Dashboard</Link></div>
    </AppLayout>
  );
  ...
  return (
    <AppLayout>
      <div ref={printRef} style={{ padding: '2rem 2.5rem', background: 'var(--bg-base)' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <Link to={`/app/sites/${analysis.site?.id}`} style={{ color: 'var(--text-muted)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 4, marginBottom: '0.5rem' }}>
              ← {analysis.site?.name}
            </Link>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', marginBottom: '0.3rem' }}>Energy Leak Analysis</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              {new Date(analysis.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })} · {analysis.site?.name}
            </p>
          </div>
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <button className="btn btn-secondary btn-sm no-print" onClick={async () => {
              try {
                const res = await api.post(`/api/reports/${analysisId}/export/share`);
            ...
            }}><Share2 size={15} /> Share Link</button>
            <button className="btn btn-secondary btn-sm no-print" onClick={handleExportCsv}><Download size={15} /> Export CSV</button>
            <button className="btn btn-secondary btn-sm no-print" onClick={handleExportPdf}><Download size={15} /> Export PDF</button>
            <Link to={`/app/analyses/${analysisId}/scenarios`} className="btn btn-primary btn-sm no-print"><Sliders size={15} /> Simulate Scenarios</Link>
          </div>
        </div>
        ...
        <div className="alert alert-info" style={{ marginBottom: '1.5rem' }}>
          ...
          <span>These are <strong>estimated</strong> figures based on your bill, schedule, and equipment profile — not audit-grade metering. Confidence: <strong>{Math.round(analysis.confidenceScore * 100)}%</strong></span>
        </div>
        ...
        <div style={{ background: 'var(--surface-dark)', borderRadius: '16px', padding: '2rem', marginBottom: '2.5rem', color: 'var(--text-inverse)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem', boxShadow: 'var(--shadow-lg)' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', marginBottom: '0.5rem', color: '#fff' }}>Your Projected Savings</h2>
            <p style={{ color: 'var(--text-inverse-muted)', fontSize: '0.95rem' }}>Implement the Kill List below to capture this value.</p>
          </div>
                ...
                <motion.div key={rec.id} className="rec-card" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.07 }}>
                  <div className="rec-card-header" onClick={() => setOpenRec(openRec === i ? null : i)}>
                    ...
                    <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
                      ...
                      {/* Key Metrics clearly visible on the card header */}
